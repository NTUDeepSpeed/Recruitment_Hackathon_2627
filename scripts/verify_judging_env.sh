...
#
# Rule 25 says not to modify the judging environment. This makes that checkable
...
if [ "${UPDATE}" = "1" ]; then
    ...
    # Pins first: judging_submodules.txt now lives inside a protected directory,
    # so the manifest has to be taken after it is rewritten, not before.
